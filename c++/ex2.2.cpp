...
pair<int, vector<int>> builder_test(vector<int> prime, int bit) {
    ...
    while(true){
        int num = rn(0, max_index);   //выбираем рандомный индекс
        int power = rn(1, max_pow);   //выбираем рандомную степень
        ...
        if(f > pow(2, (bit / 2))){      //проверяем размер 2^(bit/2)<f<2^(bit/2)+1
    ...
    do
    {
        R = rn(pow(2, (bit / 2) - 1) + 1, pow(2, bit / 2));  //рандомное 2^(bit/2)-1<R<2^bit/2
    } while (R % 2 != 0);
...
//2ая проверка
    for(int aj:a){
        bool flag = true;
        for (int qi : q) {
            if (qi != 0 && power_mod(aj, (n - 1) / qi, n) == 1) {
                flag = false;
                break;
            }
